// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'serializers.dart';

// **************************************************************************
// BuiltValueGenerator
// **************************************************************************

Serializers _$serializers = (new Serializers().toBuilder()
      ..add(Address.serializer)
      ..add(BasicStatistics.serializer)
      ..add(Call.serializer)
      ..add(ChatMessage.serializer)
      ..add(ChatMessageLike.serializer)
      ..add(ChatMessageType.serializer)
      ..add(ChatPreferences.serializer)
      ..add(ChatRoom.serializer)
      ..add(ContactInformation.serializer)
      ..add(ContactInformationValue.serializer)
      ..add(Device.serializer)
      ..add(Gender.serializer)
      ..add(Metadata.serializer)
      ..add(MetadataEvent.serializer)
      ..add(NotificationsPreferences.serializer)
      ..add(Profile.serializer)
      ..add(User.serializer)
      ..add(UserStatus.serializer)
      ..addBuilderFactory(
          const FullType(
              BuiltList, const [const FullType(ContactInformationValue)]),
          () => new ListBuilder<ContactInformationValue>())
      ..addBuilderFactory(
          const FullType(
              BuiltList, const [const FullType(ContactInformationValue)]),
          () => new ListBuilder<ContactInformationValue>())
      ..addBuilderFactory(
          const FullType(BuiltList, const [const FullType(Address)]),
          () => new ListBuilder<Address>())
      ..addBuilderFactory(
          const FullType(
              BuiltList, const [const FullType(ContactInformationValue)]),
          () => new ListBuilder<ContactInformationValue>())
      ..addBuilderFactory(
          const FullType(
              BuiltList, const [const FullType(ContactInformationValue)]),
          () => new ListBuilder<ContactInformationValue>())
      ..addBuilderFactory(
          const FullType(
              BuiltList, const [const FullType(ContactInformationValue)]),
          () => new ListBuilder<ContactInformationValue>())
      ..addBuilderFactory(
          const FullType(
              BuiltList, const [const FullType(ContactInformationValue)]),
          () => new ListBuilder<ContactInformationValue>())
      ..addBuilderFactory(
          const FullType(
              BuiltList, const [const FullType(ContactInformationValue)]),
          () => new ListBuilder<ContactInformationValue>())
      ..addBuilderFactory(
          const FullType(
              BuiltList, const [const FullType(ContactInformationValue)]),
          () => new ListBuilder<ContactInformationValue>())
      ..addBuilderFactory(
          const FullType(BuiltList, const [const FullType(String)]),
          () => new ListBuilder<String>())
      ..addBuilderFactory(
          const FullType(BuiltList, const [const FullType(ChatMessage)]),
          () => new ListBuilder<ChatMessage>()))
    .build();

// ignore_for_file: always_put_control_body_on_new_line,always_specify_types,annotate_overrides,avoid_annotating_with_dynamic,avoid_as,avoid_catches_without_on_clauses,avoid_returning_this,deprecated_member_use_from_same_package,lines_longer_than_80_chars,omit_local_variable_types,prefer_expression_function_bodies,sort_constructors_first,test_types_in_equals,unnecessary_const,unnecessary_new
