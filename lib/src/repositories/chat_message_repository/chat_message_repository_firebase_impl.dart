...
class ChatMessageRepositoryFirebaseImpl
  ...
  ChatMessageRepositoryFirebaseImpl(this.path) {
  ...
  Future addChatMessage(ChatMessage data) async {
    ref.doc(data.senderId).collection(data.receiverId).add(data.toJson());
    addToContacts(senderId: data.senderId, receiverId: data.receiverId);
    ref.doc(data.receiverId).collection(data.senderId).add(data.toJson());
    // return ref.add(data);
  }
  ...
  @override
  Future<bool> removeChatMessage(String id) async {
    await ref.doc(id).delete();
    return true;
  }

  @override
  Future<ChatMessage> getChatMessageById(String id) async {
    var doc = await ref.doc(id).get();
    return ChatMessage.fromJson(doc.data());
  }

  @override
  Future updateChatMessage(Map data, String id) async {
  ...
  Stream<BuiltList<ChatMessage>> fetchChatMessagesAsStream(
    ...
    ref
        ...
        .snapshots()
        .listen((event) => chatRoomsListController.sink.add(
            BuiltList<ChatMessage>(event.docs
 ...
 */
  ...
  @override
  DocumentReference getContactsDocument({String of, String forContact}) => _db
  ...
  Stream<BuiltList<ChatContact>> findContactStream({String userId}) {
    ...
    ref
        ...
        .listen((event) =>
            contactListController.sink.add(BuiltList<ChatContact>(event.docs
